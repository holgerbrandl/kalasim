...
Great question! Initial development was driven by curiosity about the `salabim` internals. Also, salabmin lacked (arguably) a modern touch which made some of our usecases more tricky to implement.
...
Not planned
* [Animation](https://www.salabim.org/manual/Animation.html) - which we believe should live in a separate codebase. Visualization in `kalasim` is detailed out in the [visualization chapter](visual.md).
...
2. `desim` - seemed fine initially until we discovered a the simulation engine <https://github.com/aybabtme/desim> with the same name.
...
[Kotlin-2-Java interop](https://kotlinlang.org/docs/reference/java-to-kotlin-interop.html) is a core design goal of Kotlin. Thus, kalasim should work without any issues from java. However, we have not tried yet, so in case you strugle please file a ticket.