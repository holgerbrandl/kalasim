...
object GasStation {
    ...
    val SIM_TIME = 200000.0  // Simulation time in seconds
...
//            single(qualifier = named("gas_station")) { Resource("gas_station", 2) }
            ...
            fuelPump.capacityMonitor.printStats()
            fuelPump.claimedQuantityMonitor.printStats()
            fuelPump.availableQuantityMonitor.printStats()

            println(get<GasStation>().requesters.queueLengthStats)
            println(get<GasStation>().requesters.lengthOfStayStats)
        }
    }
}